        ...
        for epoch in iterator:
            ...
            if (epoch + 1) % self.model_saving_frequency == 0:
                ...
                torch.save(
                    ...
                    # LINUX os.path.join(self.save_dir, self.model_name_prefix) + str(datetime.datetime.now())
                    os.path.join(self.save_dir, self.model_name_prefix) + '-'.join(str(datetime.datetime.now()).split(':'))
                )
        return self.metrics
